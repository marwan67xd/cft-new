...
onMounted(() => {
  if (import.meta.client && heroRef.value) {
    ...
    nextTick(() => {
      parallax(bgRef.value, heroRef.value, 22)
    })
    ...
    <div class="container relative z-10 mx-auto px-4 sm:px-6 lg:px-8 text-center mt-12 sm:mt-6 md:-mt-44 lg:-mt-52">
      <div class="py-4 px-4 sm:py-5 sm:px-6 max-w-4xl mx-auto inline-block">
        ...
        <div ref="ctaRef" class="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3">
          <NuxtLink
            :to="localePath('/tuna')"
            class="btn-dynamic inline-flex items-center justify-center px-6 py-3 rounded-xl bg-aqua-500 text-ocean-950 font-semibold hover:bg-aqua-400 transition-colors"
          >
          ...
          <a
            ...
            class="btn-dynamic inline-flex items-center justify-center px-6 py-3 rounded-xl border border-white/60 text-white font-semibold hover:bg-white/10 transition-colors"
          >
